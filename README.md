...
sequenceDiagram
    ...
    Note left of Sender: Uploads a file <br/> of binary string
    Sender->>+Channel: data having LRC, VRC, <br/> CRC & Checksum encoded data
    Channel->>+Receiver: data <br/> (may or may not be corrupted)
    Note right of Receiver: Checks the received <br/> data and reports <br/> the time taken & <br/> detection success
    Receiver->>+Sender: sends back the report with additional info
 -->
